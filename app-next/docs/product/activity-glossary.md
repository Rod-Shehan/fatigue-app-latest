...
|--------------------------|-------------|--------------------|
| Driving | **Work** | Start shift → Start driving, or Continue shift |
| Not driving, and not doing a job task — eat, drink, nap, sit still | **Rest** | Stop Driving → Start Rest |
| Not driving, but still doing a job task — load, unload, forklift, tyre, paperwork, fuel | **Other work** | Start shift → Start Other Work, or Stop Driving → Start Other Work |
...
These are taps, not extra states. **Stop Driving** and **Start shift** are not stored as those names. The record changes only when they pick a kind.

| Phrase | Role |
|--------|------|
| **Start shift** | Idle: one hero tap. Opens Set up day if needed, else the Driving / Other work chooser. Does not log by itself. |
| **Start driving** | Top half of the Start shift split. Logs Work (driving). |
...
| **Start Other Work** | Bottom half of either split. Logs Other work. |
| **Cancel** | Closes the chooser. Idle stays idle; Work stays on Work. |

**End shift** stays in the dock, never inside the split. Stop Driving = still on the job, not driving. End shift = off the job.

## Taps (locked)

Idle Start shift opens a chooser. Confirm in Set up day does **not** auto-log driving. The two kinds after Stop Driving are unchanged.
...
The split is a chooser, not a log. First tap of Start shift or Stop Driving opens it (no extra tap-again before the split). Start driving, Start Rest, and Start Other Work keep the usual tap-again confirm.
...
| Start driving | `work` | Driving. Same 7h rest gate as Start shift. |
| Continue shift | `work` | Back to driving after Rest or Other work |
| Start Rest | `break` | Existing 30 min floor: 30 min stays Rest; 31+ becomes **non-work** |
| Start Other Work | `other_work` | Break from driving on the sheet. Never convert to non-work. Counts toward 20 min / 5h. Still work time for 168h. Can start from idle (Start shift split) or after Stop Driving. |
| End shift | `stop` | Unchanged — starts **non-work** |
| Stop Driving | *(nothing)* | Chooser only |

### Reminders and voice

The 5h reminder opens the same Stop Driving split. Voice: start shift (opens Driving / Other work chooser), start driving, stop driving, start rest, start other work.
...
| Break on a driver button | **Stop Driving** then **Start Rest** or **Start Other Work** (or **Start shift** then **Start Other Work** when idle) |
