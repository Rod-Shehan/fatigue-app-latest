...
- **2. Emails & workshop** — **Checklist PDF email** (your address for Fitness for Work / Vehicle pre-departure / Trailer pre-departure / Forklift pre-departure / Load check / Hook up / Fault report PDFs — one sheet per signed log, dated **week ending**; defaults to your **sign-in email**) and **Workshop contact** (vehicle faults only — WAHVA). Not the 28-day fatigue roadside PDF.
...
4. Complete signed **Fitness for Work** before **Start shift** (**Required before Start shift**). Other **Forms** are optional — open **Vehicle pre-departure** / **Trailer pre-departure** / **Forklift pre-departure** / **Load check** / **Hook up** / **Fault report** when they match this shift (they do not block Start shift). Those three pre-departure forms stay **separate**. Week-PDF ticks (**Fitness for work**, **Daily vehicle checklist**, **Dimension & load**) show on the week PDF. Trailer, forklift, hook-up, and Fault report stay in the EWD only. After **Start Other Work** is logged, the ring keeps three tiles (**Start driving**, **Start Rest**, **Load check**); tap **Load check** again for another load. After a form is saved, use **View** to read it, or **Redo** / **Add another** for a new signed record. Vehicle pre-departure is filed under the **vehicle registration**. Load check is **one form per load** (prime + trailers/dollies on that load; six WAHVA columns). **Hook up** is one tick sheet per coupling (14 paper steps; training covers the procedure). **Produce checklist PDFs** downloads **one sheet per signed log** (Fitness for Work named by driver; vehicle and load checks by vehicle rego; Hook up by driver and rego). The date on every file is **week ending** — not signed time. Types stay separate. **Email checklist PDFs** sends those files to **your** address in **Settings → Emails & workshop → Checklist PDF email** (defaults to your sign-in email). Dimension & Load, Hook up, and Fault report can be completed more than once per day; loader CoR is separate from the driver (present sign, pending, or photo gap — no proxy). **Fault report** is a separate signed WAHVA record — a Fault tick on pre-departure does not replace it.
5. Type **start km**
