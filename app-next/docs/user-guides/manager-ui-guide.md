...
| **Rego** | Vehicle catalogue: plate, **Type** (Prime mover / Rigid / Van / Trailer / Other), **GVM (t)** and **GCM (t)** on powered units, **ATM (t)** on trailers, **Tare (t)** on every unit, **Number of axles**, and **WAHVA Accredited** (RAV permit vehicle). Required when adding or editing a rego. Drivers pick the plate on Set up day; the masses that apply show next to the plate on the day sheet for load check. A **Prime mover** plate highlights **Hook up** as a reminder (not required). |
