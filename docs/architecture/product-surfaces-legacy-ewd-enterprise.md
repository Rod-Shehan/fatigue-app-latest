...
|------|---------|---------|
| `helper.circadia24.com` | `legacy` | **Named Helper host** — combined driver + manager (parked original) |
| `legacy.circadia24.com` | `legacy` | Old Helper name. Same surface; do not treat as Staff Desk. |
...
`helper.circadia24.com` is attached on the `fatigue-app-latest` Vercel project. Unknown hosts (including `localhost`) still default to **legacy** / Helper behaviour. That fallback is not Helper’s address. `www.circadia24.com` is the marketing site, not Helper.
...
```bash
APP_SURFACE=legacy|helper|ewd|enterprise|circadia
NEXT_PUBLIC_APP_SURFACE=legacy|helper|ewd|enterprise|circadia   # lobby badge if Host inference unavailable
NEXTAUTH_URL=https://<that-host>/

# Cross-links when a path belongs on another product:
NEXT_PUBLIC_HELPER_APP_URL=https://helper.circadia24.com
NEXT_PUBLIC_LEGACY_APP_URL=https://helper.circadia24.com   # older name; Helper URL wins if both set
...
If `APP_SURFACE` / `NEXT_PUBLIC_APP_SURFACE` are unset, middleware and the home page infer from `Host` (`helper.` / `legacy.` / `ewd.` / `enterprise.` / `staff-desk.`). Otherwise default is **legacy** (Helper). The `staff-desk.` host always maps to **circadia**, even when `APP_SURFACE=legacy` on the shared Vercel project. The old `admin.` host 308s to staff-desk.
...
1. Add domain aliases: `helper`, `ewd`, `enterprise`, `staff-desk` on the appropriate Vercel project(s). `legacy` may stay as an alias to Helper.
2. Set `APP_SURFACE` + `NEXTAUTH_URL` (+ sibling `NEXT_PUBLIC_*_APP_URL`) per host. For `helper.circadia24.com` and `staff-desk.circadia24.com` on the shared project, Host inference is enough. Do **not** redirect `www.circadia24.com/circadia` to the staff desk; that host is the marketing site. Random non-desk paths on staff-desk go to `https://www.circadia24.com`.
3. Keep current `www` on the unsplit fallback until EWD/Enterprise are verified. Do not treat `www` as Helper’s named address.
4. Point drivers at EWD; managers at Enterprise; Circadia staff at `staff-desk.circadia24.com`; Helper at `helper.circadia24.com`. Customer Owner tools stay at `/admin` on Enterprise.
