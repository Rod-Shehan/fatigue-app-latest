...
async function deleteOneNoVideoIncident(tx: TxClient, lifecycleId: string): Promise<"deleted" | "has_video" | "not_found"> {
  ...
  if (hasViewableVideoClip(clip)) return "has_video";
...
export async function deleteNoVideoIncidents(
  ...
  for (const lifecycleId of uniqueIds) {
    try {
      const result = await deleteOneNoVideoIncident(tx, lifecycleId);
